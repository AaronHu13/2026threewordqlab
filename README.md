...
| 幕 | cue 数 | 说明 |
|---|---|---|
| Act 1 回来了 | 11 | 报幕 + 7 条音效，顺序播；3 careless whisper（2s）/ 5 常回家看看（10s）/ 6 我记得（10s）各配一条手动淡出（5 / 6 的 10s 是 2026-09-16 剧组要求）。09-18 加 7 咔嚓（0.5s）：放在 1_6 和 1_6F 之间，6 播放中手动 GO、叠着放一声，不影响 6 |
| Act 2 这个童话不对劲 | 27 | 报幕 + 11 条音效 + 11 条旁白。2026-09-17 剧组新增 23 结尾垫乐（21s，编号沿用剧组顺序号，接在旁白11 后）+ 一条 3s 手动淡出（剧组说的 cue 24）；**09-18 zhabro 在总工程里删掉 9 机器人启动**（文件仍在文件夹里）。**没有拿到 cue 表**，音效按文件编号顺序、旁白按剧情插在 10 小矮人家 / 11 吃苹果前后，1 魔幻森林 / 4 皇后 intro / 6 狼人杀 各配一条手动淡出。顺序需要剧组确认，改 json 重建即可 |
| Act 3 今天小神有点忙 | 18 | 报幕 + 17 条，cue 13/14 音量 +3.5 dB；**09-18 zhabro 在总工程里删掉 07 sidapusa / 08 dazhuanpan 的两条手动淡出**，全部播完 |
| Act 4 卡巴莱特新编 | 14 | 报幕 + 11 条；1 In car（3s，**09-18 zhabro 压到 −8.05 dB**）/ 12 kickline（8s）各配一条手动淡出（时长是 2026-09-17 码戏剧组要求）。**09-18 zhabro 在总工程里把 9 新闻连同四版候选全部删掉**（音频仍在文件夹和 `9 候选（09-17 码戏四选一）/` 里），3 锣鼓intro 换成 4.6s 新版（旧版 6.8s） |
| Act 5 大堡健 | 20 | 报幕 + 17 条音效 + 2 条淡出 |
| Act 6 最后的太阳 | 36 | 2026-09-15 按新音频包 + `requirements.txt` 重建：报幕 + 21 条音效，含 9+10 一键同停、12+13 一键同起、cue 8 / cue 18 播放中压低的 D 类 fade。Cue 13 换 mp3、Cue 15 剪掉 1-2-3 倒数、Cue 16 换 35.8s 新版。**2026-09-16 剧组微信回传的独立工程已回收**：手动淡出全改 1s（9F 1s 但 10F 仍 2s，照原样保留）、8D 从压到 65% 改为压到 30%（-13.6 dB）、cue 11 / 14 换 `… New.mp3` 新录音（旧版在 `原始素材/`）、cue 17 +3 dB。**2026-09-17 cue 3 神剑登场换剧组 plus 版**（7.6s → 9.0s，响度不变，同名覆盖，旧版在 `原始素材/`）；09-18 cue 5 The King 加一条 1s 手动淡出 |
