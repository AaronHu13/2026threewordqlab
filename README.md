...
| Act 4 卡巴莱特新编 | 22 | 报幕 + 12 条；1 In car（3s）/ 9 long新闻（2s）/ 12 kickline（8s）各配一条手动淡出（1 / 12 的时长是 2026-09-17 码戏剧组要求，12 的 8s 可能还会调）。**9 新闻 09-17 码戏四选一**：4_9 long（66s，现用）/ 4_91 mid（47s）/ 4_92 short + 长掌声（35s）/ 4_93 short（31s）四条各带 2s 手动淡出，91–93 的音频在 `9 候选（09-17 码戏四选一）/`；定稿后只留一条、删掉另外三条并把 cue 数改回 16 |
